...
module.exports = class Builder {
  ...
  build() {
    ...
    return (0, _asyncToGenerator3.default)(function* () {
      ...
      if (_this._nuxtPages) {
        if (!existsSync(join(_this.options.srcDir, 'pages'))) {
  ...
  generateRoutesAndFiles() {
    ...
    return (0, _asyncToGenerator3.default)(function* () {
      ...
      const templateVars = {
        ...
        middleware: existsSync(join(_this2.options.srcDir, 'middleware')),
        ...
        // -- Layouts --
      };if (existsSync(resolve(_this2.options.srcDir, 'layouts'))) {
        const layoutsFiles = yield glob('layouts/**/*.{vue,js}', { cwd: _this2.options.srcDir, ignore: [`layouts/**/${_this2.options.ignorePrefix}*.{vue,js}`] });
      ...
      if (_this2._nuxtPages) {
        ...
        const files = {};
        (yield glob('pages/**/*.{vue,js}', { cwd: _this2.options.srcDir, ignore: [`pages/**/${_this2.options.ignorePrefix}*.{vue,js}`] })).forEach(function (f) {
          const key = f.replace(/\.(js|vue)$/, '');
          if (/\.vue$/.test(f) || !files[key]) {
            files[key] = f;
          }
        });
        templateVars.router.routes = createRoutes((0, _values2.default)(files), _this2.options.srcDir);
      } else {
  ...
  watchFiles() {
    ...
    let patterns = [r(src, 'layouts'), r(src, 'store'), r(src, 'middleware'), r(src, 'layouts/*.{vue,js}'), r(src, 'layouts/**/*.{vue,js}')];
    if (this._nuxtPages) {
      patterns.push(r(src, 'pages'), r(src, 'pages/*.{vue,js}'), r(src, 'pages/**/*.{vue,js}'));
    }
    ...
    this.customFilesWatcher = chokidar.watch(watchFiles, options).on('change', refreshFiles);
  }
