...
var _require3 = require('../common/utils'),
    ...
    isPureObject = _require3.isPureObject;
...
Options.from = function (_options) {
  ...
  var options = (0, _assign2.default)({}, _options);
  ...
  // Populate modulesDir
  options.modulesDir = [].concat(options.modulesDir).concat(join(options.nuxtDir, 'node_modules')).filter(function (dir) {
    return hasValue(dir);
  }).map(function (dir) {
    return resolve(options.rootDir, dir);
  });
  ...
  if (options.store !== false && existsSync(join(options.srcDir, options.dir.store)) && readdirSync(join(options.srcDir, options.dir.store)).find(function (filename) {
