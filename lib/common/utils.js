...
exports.waitFor = function waitFor(ms) {
  return new _promise2.default(resolve => setTimeout(resolve, ms || 0));
};
...
exports.sequence = function sequence(tasks, fn) {
  return tasks.reduce((promise, task) => promise.then(() => fn(task)), _promise2.default.resolve());
};

exports.parallel = function parallel(tasks, fn) {
  return _promise2.default.all(tasks.map(task => fn(task)));
};
...
exports.createRoutes = function createRoutes(files, srcDir, pagesDir) {
  let routes = [];
  files.forEach(file => {
    let keys = file.replace(RegExp(`^${pagesDir}`), '').replace(/\.(vue|js)$/, '').replace(/\/{2,}/g, '/').split('/').slice(1);
