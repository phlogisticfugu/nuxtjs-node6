...
const Vue = _interopRequireDefault(_vue);

var _router = require('vue-router');
const Router = _interopRequireDefault(_router);
...
uniqBy(_components, '_name').forEach((route) => { %>const <%= route._name %> = () => require('<%= relativeToBuild(route.component) %>' /* webpackChunkName: "<%= wChunk(route.chunkName) %>" */).then(m => m.default || m)
<% }) %>
